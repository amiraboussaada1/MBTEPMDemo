...
public abstract class HeaderProviderBase {
    public mbtepmdemo.MainServlet servlet;
    public mbtepmdemo.proxy.ComSapMbtepmdemoService service;

    private static final StringSet PROPAGATE_HEADERS = mbtepmdemo.HeaderProvider.initPropagate();
    ...
    public final void addHttpHeaders(HttpDestination destination, org.apache.http.HttpRequest request) {
        HttpHeaders headers = new HttpHeaders();
        addHttpHeaders(destination, request, headers);
        CopyHeaders.toRequest(headers, request);
    }

    public void addHttpHeaders(HttpDestination destination, org.apache.http.HttpRequest request, HttpHeaders headers) {
    ...
    public void onHttpResponse(HttpDestination destination, org.apache.http.HttpResponse response, HttpHeaders headers) {
    }

    public void addAuthorization(HttpDestination destination, org.apache.http.HttpRequest request, HttpHeaders headers) {
        destination.addAuthorization(servlet, request, headers);
    }

    public void addConnectivity(HttpDestination destination, org.apache.http.HttpRequest request, HttpHeaders headers) {
